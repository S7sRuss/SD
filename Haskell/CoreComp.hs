...
assocAlign :: (Env Dest,Env Dest, Int) -> (String,Type) -> (Env Dest,Env Dest, Int)
assocAlign = \(env,cur,start) -> 
             \(y,t) -> 
             let (this,next) = nextAlign t start in 
                 (env, assocLast cur y $ REnv y t this, next)
...
op2Str (Phi IntType d d' l' d'' l'') = "  "++dest2Str d++" = phi i32 ["++
...
op2Str (Call_type (FunType t t') d d' d'') = 
       "  "++dest2Str d++
       " = call "++type2LLVM t'++" @closure_call"++type2Str (FunType t t')++
       "(%struct.closure_type* "++
       dest2Str d'++", "++
       type2LLVM t++" "++dest2Str d''++") nounwind ssp"

op2Str (Get_env t d d' d'' y n) = 
  "; get "++y++" align "++show n++"\n"++
  "  "++dest2Str d++" = getelementptr inbounds i8* %env, i32 "++show n++"\n"++
  "  "++dest2Str d'++" = bitcast i8* "++dest2Str d++" to "++type2LLVM t++"*\n"++
  "  "++dest2Str d''++" = load "++type2LLVM t++"* "++dest2Str d'++", align "++show (alignOf t)++"\n"

op2Str (Create_fn (FunType t t') d d' fn args) = 
  "  "++dest2Str d'++" = call i8* @create_env_f"++show fn++
  "("++argList args++") nounwind ssp\n"++
  "  "++dest2Str d++" = call %struct.closure_type* @closure_create(i8* "++
  dest2Str d'++", void ()* bitcast ("++type2LLVM t'++" ("++type2LLVM t++", i8*)* @f"++show fn++" to void ()*)) nounwind"
...
comp (Add e e') env (start,end) = 
     ...
     d'' <- newReg
     return ( concat [ bs, bs', [IBlock m' [Add_i32 d'' d d'] end]], fs ++ fs', List.union ts ts', d'',end)
...
comp (Sub e e') env (start,end) = 
     ...
     (bs',fs',ts',d',m') <- comp e' env (m,m') 
     d'' <- newReg
     return ( concat [ bs, bs', [IBlock m' [Sub_i32 d'' d d'] end]], fs ++ fs', List.union ts ts', d'',end)
...
comp (Mul e e') env (start,end) = 
     ...
     (bs',fs',ts',d',m') <- comp e' env (m,m') 
     d'' <- newReg
     return ( concat [ bs, bs', [IBlock m' [Mul_i32 d'' d d'] end]], fs ++ fs', List.union ts ts', d'',end)
...
comp (Div e e') env (start,end) = 
     ...
     (bs',fs',ts',d',m') <- comp e' env (m,m') 
     d'' <- newReg
     return ( concat [ bs, bs', [IBlock m' [Div_i32 d'' d d'] end]], fs ++ fs', List.union ts ts', d'',end)
...
comp (Assign e e' t) env (start,end) = 
     ...
     (bs,fs,ts,d,m) <- comp e env (start,m)
     (bs',fs',ts',d',m') <- comp e' env (m,m') 
     return (concat [bs, bs', [IBlock m' [Set_var t d d'] end]], fs ++ fs', List.union ts ts', d',end)
...
comp (Equal e e') env (start,end) = 
     ...
     (bs',fs',ts',d',m') <- comp e' env (m,m') 
     d'' <- newReg
     return ( concat [ bs, bs', [IBlock m' [Eq_i32 d'' d d'] end]], fs ++ fs', List.union ts ts', d'',end)
...
comp (While e_c e_b) env (start,end) =      
     ...
     return (concat [bs, bs', [CBlock l_cond [] (d,l_body,end), 
                               IBlock l_exit [] start]], fs ++ fs', List.union ts ts', d',end)
...
{- TODO: Can we save 2 labels? t_then_exit and l_else_exit ? -}
...
comp (If e_c e_t e_e t) env (start,end) = 
     ...
     let all_bs = concat [bs,bs',bs'',
            ...
            [IBlock l_exit [Phi t d''' d' l_then_exit d'' l_else_exit] end]] in 
                    return (all_bs, fs ++ fs' ++ fs'', List.union (List.union ts ts') ts'', d''', end)
...
comp (Fun x _ e t) env (start,end) = 
     ...
     let (_,nenv_free,mallocSize) = foldl assocAlign (env,[],0) envnames in 
       -- build env
       do
       fn <- newFunName   
...
dump :: [Block] -> [Function] -> [Type] -> Dest -> Label -> String
dump bs fs ts d end = 
     file_header ++ 
     dump_types ts ++
     dump_functions fs ++
...
compState e = let SM(f) = comp e [] entryresult in 
                  case snd (f emptyState) of
                       Just (bs,fs,ts,d,end) -> Just $ dump bs fs ts d end
                       Nothing -> Nothing 
...
zz = Decl "x" (Num 0) (Decl "x" (Num 1) (Add (Id "x" None) (Id "x" None)) None) None
