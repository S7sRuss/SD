...
let rec aux_p_list_type l = 
	match l with
	| (a,_)::t -> a^","^(aux_p_list_type t)
	| [] -> ""

let print_list_type l =
	let s = aux_p_list_type l in
		let len = String.length s in
			if len=0 then "[]"
			else let substring = (String.sub s 0 (len-1)) in
				"["^substring^"]"
...
let rec unparse a idt =
	let ni = idt^"  " in
	match a with
		
		| Number n -> string_of_int n
		| Add (l,r) -> "Add("^(unparse l idt)^","^(unparse r idt)^")"
		| Sub (l,r) -> "Sub("^(unparse l idt)^","^(unparse r idt)^")"
		| Mul (l,r) -> "Mul("^(unparse l idt)^","^(unparse r idt)^")"
		| Div (l,r) -> "Div("^(unparse l idt)^","^(unparse r idt)^")"

		| Id (x,_) -> x
		| Decl (x,e1,e2,_) -> "Decl "^x^" = "^(unparse e1 ni)
							^"\n"^ni^"in "^(unparse e2 ni)

		| Var (x,_) -> "Var("^(unparse x idt)^")"
		| Deref (x,_) -> "!("^(unparse x idt)^")"
		| Assign (x,v,_) -> "Assign("^(unparse x idt)^":="^(unparse v idt)^")"
		| Free (x) -> "Free("^(unparse x idt)^")"

		| If (c,i,e,_) -> 
			"If("^(unparse c idt)^")"
			^"\n"^ni^" then "^(unparse i ni)
			^"\n"^ni^" else"^(unparse e ni)
		| While (c,e) -> "While("^(unparse c idt)^")"
			^"\n"^ni^"do "^(unparse e ni)^")"

		| Seq (e1,e2) -> "Seq("^(unparse e1 idt)^";"^(unparse e2 idt)^")"
		| Skip -> "Skip"

		| Eq (e1,e2) -> "Equal( "^(unparse e1 idt)^" == "^(unparse e2 idt)^" )"
		| Not (e1) -> "Not( "^(unparse e1 idt)^" )"

		| Fun (s,t1,e1,t2) -> "Fun("^s^":"^(string_of_type t1)^" -> "^(unparse e1 idt)^")"
		| Call (e1,e2,_) -> "Call(("^(unparse e1 idt)^")("^(unparse e2 idt)^"))"
